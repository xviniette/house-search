...
function extractSeLoger() {
  ...
  const yearM = raw.match(/Année de construction[\\",]*[\\",]*value[\\",]*[\\",]*(\d{4})/i)
             || text.match(/Année de construction\s*(\d{4})/i);
  if (yearM) data.annee_construction = yearM[1];

  const consoM = text.match(/(\d+)\s*kWh\/m[²2][\.\s]*an/i)
              || raw.match(/(\d+)\s*kWh\/m/);
  if (consoM) data.conso = parseInt(consoM[1], 10);

  const gesM = text.match(/(\d+)\s*kg\s*CO[₂2]\/m[²2]/i)
            || raw.match(/(\d+)\s*kg\s*CO/);
